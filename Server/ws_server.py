...
async def main():
    ...
    if not cfg.WS_TOKEN:
        ws_error(
            "WS_SERVER",
            "No WebSocket server token found in .env file",
    ...
    if not skip_npm:
        ...
        try:
            ...
            if not npm_ready:
                ws_error(
                    "WS_SERVER",
                    "NPM container is not accessible",
                    suggestions=[
                ...
                ws_warning(
                    "WS_SERVER",
                    "Server will start anyway but streams may not work properly",
                )
        ...
        if port_in_use:
            ...
            try:
                ...
                if processes:
                    ...
                    ws_warning(
                        "WS_SERVER",
                        f"Process(es) using port {cfg.WS_SERVER_PORT}:",
                        process_details,
                    )
                    ...
                    for pid, command in processes:
                        if "ws_server.py" in command or (
                            "python" in command.lower() and "ws_server" in command
                        ):
                    ...
                    if our_processes:
                        ws_info(
                            "WS_SERVER",
                            f"Found {len(our_processes)} likely WebSocket server process(es)",
                        )
                        ws_warning(
                            "WS_SERVER",
                            "Attempting to terminate existing WebSocket server processes...",
                        )
                            ...
                            try:
                                if platform.system().lower() == "windows":
                                    ...
                                    if result.returncode == 0:
                                        ...
                                        ws_error(
                                            "WS_SERVER",
                                            f"Failed to terminate process {pid}: {result.stderr}",
                                        )
                                else:
                                    os.kill(int(pid), 9)
                        ...
                        if terminated_count > 0:
                            ...
                            ws_info(
                                "WS_SERVER",
                                "Waiting 5 seconds for ports to be released...",
                            )
                            time.sleep(5)
                            ...
                            if not pu.is_port_in_use(cfg.WS_SERVER_PORT):
                                ws_success(
                                    "WS_SERVER",
                                    f"Port {cfg.WS_SERVER_PORT} is now available",
                                )
                            else:
                                ws_error(
                                    "WS_SERVER",
                                    f"Port {cfg.WS_SERVER_PORT} is still in use after termination attempts",
                                )
                                ws_warning(
                                    "WS_SERVER",
                                    "Will attempt to start server anyway...",
                                )
                        else:
                            ws_error(
                                "WS_SERVER",
                                "Failed to terminate any existing processes",
                            )
                    else:
                        ws_warning(
                            "WS_SERVER",
                    ...
                    ws_warning(
                        "WS_SERVER",
                        f"Could not identify the process using port {cfg.WS_SERVER_PORT}",
                    )
    ...
    try:
        ...
        # Asegúrate que docker_utils.periodic_cleanup() NO reinicie NPM ni los contenedores
        cleanup_task = asyncio.create_task(docker_utils.periodic_cleanup())
                            ...
                            if last_seen:
                                try:
                                    if isinstance(last_seen, (int, float)):
                                        last_seen_fmt = datetime.datetime.fromtimestamp(
                                            last_seen
