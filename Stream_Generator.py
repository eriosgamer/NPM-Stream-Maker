...
def main():
    # 1. Read required ports
    required_ports = load_ports(PORTS_FILE)

    # Exclude ports reserved by NPM
    required_ports = required_ports - RESERVED_PORTS

    # 2. Read existing streams and proxy hosts from the SQLite database
    streams_ports = ports_in_streams_sqlite()
    proxy_ports = ports_in_proxy_host_sqlite()

    # 3. Determine missing ports/protocols
    # Group by port so each port only has one entry (with both protocols if needed)
    missing_by_port = {}
    for port in sorted(required_ports):
        ...
        if tcp_missing or udp_missing:
            missing_by_port[port] = []
            if tcp_missing:
                missing_by_port[port].append("tcp")
            if udp_missing:
                missing_by_port[port].append("udp")

    new_entries = []
    for port, protos in missing_by_port.items():
        for proto in protos:
            new_entries.append((port, proto))

    print(f"{len(missing_by_port)} new stream entries will be generated for NPM.")

    # 4. Create .conf files for the new streams
    if new_entries:
        os.makedirs(NGINX_STREAM_DIR, exist_ok=True)
        # Do not create files here, only after syncing with the database

        # 5. Add to the SQLite database (only once per port)
        add_streams_sqlite(new_entries)
        ...
        print("No new entries to add. Container will not be restarted.")

    # Sync the .conf files with the database after adding new streams
