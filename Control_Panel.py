    ...
    if not token:
        if mode == "server":
            ...
            with open(TOKEN_FILE, "w") as f:
                f.write(token)
            console.print(
                f"[green]WebSocket token generated and saved in {TOKEN_FILE}[/green]")
            console.print(f"[yellow]Token: [bold]{token}[/bold][/yellow]")
        elif mode == "client":
            token = Prompt.ask(
                "[bold cyan]Paste the WebSocket token generated by the server[/bold cyan]")
            with open(TOKEN_FILE, "w") as f:
                f.write(token)
...
def run_script(script):
    ...
    if script == "ws_server.py":
        ...
        while True:
            ...
            if not token:
                # Cargar token desde archivo si no está en env
                if os.path.exists(TOKEN_FILE):
                    with open(TOKEN_FILE, "r") as f:
                        token = f.read().strip()
            if not token:
            ...
            if test_ws_connection(uri, token):
                save_ws_uri(uri)
                console.print(
                    f"[green]Connection successful. URI saved for future use.[/green]")
                break
            else:
                console.print(
                    f"[red]Connection failed or invalid token. Please try again.[/red]")
        ...
        with open(ENV_FILE, "r") as f:
            for line in f:
                if line.startswith("WS_URI="):
                    ...
                    break
    # Añadir protección para ejecución solo desde el panel
...
"""
        ...
        console.print(
            f"[green]Basic docker-compose.yml for Nginx Proxy Manager created at: {compose_path}[/green]")
        console.print("[yellow]To start Nginx Proxy Manager, run:[/yellow]")
        console.print(
            f"[bold cyan]cd {npm_dir} && docker-compose up -d[/bold cyan]")
    else:
        console.print(
            f"[yellow]A docker-compose.yml file already exists at {compose_path}[/yellow]")
...
def check_npm(console):
    ...
    try:
        ...
        console.print(
            f"[red]Could not check Nginx Proxy Manager instance: {e}[/red]")


def main():
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--ws-client-only", action="store_true", help="Run only ws_client.py without dependency checks")
    args = parser.parse_args()

    console = Console()
    if args.ws_client_only:
        # Solo ejecutar ws_client.py, sin comprobaciones
    ...
    while True:
        ...
        option = Prompt.ask("[bold yellow]Select an option", choices=[
                            str(i) for i in range(1, len(SCRIPTS)+1)])
        idx = int(option) - 1
