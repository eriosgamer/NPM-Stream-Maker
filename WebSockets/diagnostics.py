                        ...
                        if token_result.get("status") == "ok":
                            ...
                            if capabilities:
                                ...
                                if has_wg:
                                    ...
                                    peer_ip = capabilities.get("wireguard_peer_ip")
                                    ws_info(
                                        "[WS_CLIENT]",
...
def get_ws_uris_and_tokens():
    ...
    if not uris:
        ...
        ws_info(
            "[WS_CLIENT]",
            "[bold yellow]⚠️  Assuming first startup or server-only usage[/bold yellow]",
        )
        return []
    ...
    ws_info(
        "[WS_CLIENT]",
        f"[bold green] Configured {len(uri_token_pairs)} URI-token pairs:[/bold green]",
    )
    ...
    for i, (uri, token) in enumerate(uri_token_pairs, 1):
        ...
        ws_info(
            "[WS_CLIENT]",
            f"[bold white]  {i}. {uri} (token: {masked_token})[/bold white]",
        )
