...
def get_listening_ports_with_proto():
    ...
    try:
        ...
        if len(unique_ports) < 3:
            console.print("[bold yellow][PORT_DETECTION][/bold yellow] ⚠️ Se detectaron pocos puertos. Puede haber un problema de permisos o el proceso no tiene privilegios suficientes.")
        console.print(
...
async def process_new_ports_with_discovery(local_ip, hostname, new_ports):
    ...
    """
    console.print(
        f"[bold cyan][WS_CLIENT][/bold cyan] Processing {len(new_ports)} new ports with server discovery..."
    )
    ...
    if not conflict_resolution_servers:
        console.print(
            f"[bold red][WS_CLIENT][/bold red] No conflict resolution servers available"
        )
        return False
    ...
    if not response:
        console.print(
            f"[bold red][WS_CLIENT][/bold red] Failed to process ports through conflict resolution"
        )
        return False
    ...
    console.print(
        f"[bold cyan][WS_CLIENT][/bold cyan] Processed {len(results)} port results"
    )
    if conflict_resolutions:
        console.print(
            f"[bold yellow][WS_CLIENT][/bold yellow] Received {len(conflict_resolutions)} conflict resolutions"
        )
    ...
    for result in results:
        ...
        if port:
            ...
            if conflict_resolved:
                console.print(
                    f"[bold yellow][WS_CLIENT][/bold yellow] Port {port} ({proto}) assigned alternative incoming port: {incoming_port}"
                )
            else:
                console.print(
                    f"[bold green][WS_CLIENT][/bold green] Port {port} ({proto}) assigned normally"
                )
