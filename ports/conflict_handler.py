...
def get_conflict_resolution_info():
    ...
    try:
        ...
        console.print(
            f"[bold red][STREAM_MANAGER][/bold red] Error getting conflict resolution info: {e}")
    finally:
        conn.close()
...
def find_conflict_resolution_by_multiple_ips(original_port, protocol, client_ips):
    ...
    try:
        ...
        console.print(
            f"[bold red][STREAM_MANAGER][/bold red] Error finding conflict resolution: {e}")
        return None
    finally:
        conn.close()
...
async def notify_clients_of_conflicts_and_assignments():
    ...
    """
    # Copy the dictionary to avoid RuntimeError if it changes during iteration
    connected_clients_copy = dict(cfg.connected_clients)

    # Build a mapping: (port, proto) -> [client_id, ...]
    port_map = {}
    ...
    # Assign ports: first client to report gets the port, others get an alternative
    new_assigned_ports = {}
    port_conflicts = {}
    ...
    # Copy again to avoid changes during iteration
    connected_clients_copy = dict(cfg.connected_clients)
    for client_id, info in connected_clients_copy.items():
        assigned = []
        conflicts = []
        ...
        # Notify client of assignments and conflicts
        try:
            ws = info["ws"]
    ...
    for port_proto, clients in port_map.items():
        if len(clients) == 1 and prev_assigned_ports.get(port_proto) != clients[0]:
            client_id = clients[0]
            ...
            if info:
                try:
                    ...
                    # Change Console.print to console.print and check websocket closed attribute correctly
                    try:
                        if hasattr(ws, 'closed') and not ws.closed:
                            console.print(
                                f"[bold red][WS][/bold red] Error notifying client {client_id} of port assignment update: {ex}")
                    except Exception:
        ...
        if len(clients) > 1 and prev_assigned_ports.get(port_proto) != clients[0]:
            ...
            if info:
                try:
                    ...
                    # Similar fix for multiple conflicts
                    try:
                        if hasattr(ws, 'closed') and not ws.closed:
                            console.print(
                                f"[bold red][WS][/bold red] Error notifying client {client_id} of conflict update: {ex}")
                    except Exception:
                        logging.debug(f"Error checking websocket status for client {client_id}: {ex}")
        ...
        if len(clients) > 1 and prev_assigned_ports.get(port_proto) != clients[0]:
            ...
            if info:
                try:
                    ...
                    console.print(f"[bold red][WS][/bold red] Error notifying client {client_id} of port conflict resolution: {ex}")
