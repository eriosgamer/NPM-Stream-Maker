...
def start_ws_client():
    ...
    if not uris or not tokens or not any(tokens):
        ...
        console.print(
            "[bold yellow]Please use option 3 (Edit WebSocket URIs) to configure servers and tokens first.[/bold yellow]"
        )
        input("\nPress Enter to continue...")
        return

    # Test connections before starting
    console.print("[bold cyan]Testing connections to configured servers...[/bold cyan]")
...
def is_ports_file_outdated():
    ...
    try:
        ...
        console.print(
            f"[bold yellow][WS_CLIENT][/bold yellow] Error checking ports file: {e}"
        )
        return True
...
def ensure_ports_file():
    ...
    if needs_update:
        console.print(
            "[bold yellow][WS_CLIENT][/bold yellow] Ports file needs update, generating..."
        )
        try:
            ...
            if result.returncode == 0:
                console.print(
                    "[bold green][WS_CLIENT][/bold green] Ports file generated successfully"
                )
            else:
                console.print(
                    f"[bold red][WS_CLIENT][/bold red] Failed to generate ports file: {result.stderr}"
                )
        except Exception as e:
            console.print(
                f"[bold red][WS_CLIENT][/bold red] Error generating ports file: {e}"
            )
    else:
...
async def send_ports_on_connect(ws):
    """
    Envía la lista de puertos activos al servidor tras conectar o reconectar.
    """
    try:
        # Suponiendo que tienes una función o variable que da los puertos activos:
        # Ejemplo: cfg.client_assignments contiene los puertos
        ports = []
        ...
        console.print(
            f"[bold red][WS_CLIENT][/bold red] Error enviando puertos tras reconexión: {e}"
        )
...
async def main():
    ...
    """
    console.print(
        "[bold green][WS_CLIENT][/bold green] Starting WebSocket client with server discovery..."
    )

    try:
        await wscth.ws_client_main_loop(on_connect=send_ports_on_connect)
    except KeyboardInterrupt:
...
def save_client_assignments():
    ...
    try:
        ...
        console.print(
            f"[bold red][WS_CLIENT][/bold red] Error saving client assignments: {e}"
        )
...
def load_client_assignments():
    ...
    try:
        if os.path.exists(cfg.CLIENT_ASSIGNMENTS_FILE):
            ...
            console.print(
                f"[bold green][WS_CLIENT][/bold green] Loaded {len(client_assignments)} client assignments"
            )
    except Exception as e:
        console.print(
            f"[bold red][WS_CLIENT][/bold red] Error loading client assignments: {e}"
        )
